...
import { Timer, Flame, Users, ChefHat, Heart, Share2, ShoppingBasket, Utensils } from 'lucide-react';
import { MOCK_RECIPES } from '../types';

export function RecipeDetailPage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const recipe = MOCK_RECIPES.find(r => r.id === id) || MOCK_RECIPES[3];
        ...
        <div className="md:col-span-5 lg:col-span-6 relative">
          <div className="relative h-[512px] md:h-[calc(100vh-120px)] w-full overflow-hidden md:rounded-r-3xl md:mt-4">
            ...
            <div className="absolute top-4 right-4 flex flex-col gap-3 md:hidden">
              <button className="w-10 h-10 bg-surface/90 backdrop-blur-md rounded-full flex items-center justify-center text-primary shadow-lg">
                <Heart size={20} />
              </button>
                ...
                {recipe.ingredients.map((ing, i) => (
                  <li key={i} className="group flex items-center justify-between p-4 bg-surface-container-lowest rounded-xl hover:shadow-md transition-shadow">
                    <label className="flex items-center gap-4 cursor-pointer w-full">
                      ...
                      <span className="text-sm text-on-surface group-hover:text-primary transition-colors">{ing.item}</span>
                    </label>
