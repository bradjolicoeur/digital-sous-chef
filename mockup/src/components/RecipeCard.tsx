      ...
      <Link to={`/recipe/${recipe.id}`} className="group">
        <div className="aspect-[3/4] rounded-2xl overflow-hidden mb-4 relative">
          <img 
            ...
            referrerPolicy="no-referrer"
          />
          <button className="absolute top-3 right-3 w-8 h-8 bg-white/80 backdrop-blur rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
  ...
  return (
    ...
    >
      <div className="aspect-[4/3] relative overflow-hidden">
        ...
        />
        {recipe.id === '1' && (
          <div className="absolute top-4 right-4 bg-surface/90 backdrop-blur px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest text-primary">
            Imported Today
          </div>
        )}
      </div>
